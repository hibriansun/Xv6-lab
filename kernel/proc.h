...
struct cpu {
  ...
  struct context context;     // swtch() here to enter scheduler().         // TODO ??? 上下文
  int noff;                   // Depth of push_off() nesting.               // 某个CPU核心上中断嵌套计数
  int intena;                 // Were interrupts enabled before push_off()? // 关闭中断前中断开启状态
};
